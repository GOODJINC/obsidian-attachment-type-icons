...
기본 아이콘 위치를 사용할 경우 읽기 화면에서는 다음과 같이 표시됩니다.
...
**설정 → Attachment Type Icons**에서 기본 유형을 변경하거나 사용자 유형을 직접 추가할 수 있습니다.

- **활성화/비활성화:** 유형별 아이콘 표시 여부를 토글로 변경합니다.
- **유형 이름 변경:** 작업 방식에 맞는 이름을 지정합니다.
- **아이콘 변경:** 원하는 이모지 또는 문자로 바꿉니다.
- **확장자 편집:** 쉼표로 구분해 확장자를 추가하거나 삭제합니다.
- **폴더 지정:** 입력을 시작하면 보관함의 폴더가 추천되며, 경로를 직접 입력할 수도 있습니다. 폴더가 지정된 유형은 전체 폴더에 적용되는 유형보다 우선합니다.
- **우선순위 설정:** 위·아래 버튼으로 유형 순서를 변경합니다. 폴더 지정 유형과 일반 유형의 각 그룹에서는 먼저 일치하는 유형이 적용됩니다.
- **아이콘 위치:** 링크 텍스트의 앞이나 뒤를 선택합니다.

설정 화면은 Obsidian의 앱 언어를 따릅니다. Obsidian이 한국어로 설정되어 있으면 한국어를 표시하고, 그 밖의 언어에서는 영어를 표시합니다.

**Add file type** 버튼을 사용하면 `📚` 아이콘과 `epub, mobi, pdf` 확장자를 사용하는 `Ebooks` 같은 유형을 만들 수 있습니다. 사용자가 추가한 유형은 설정 화면에서 삭제할 수도 있습니다.

### 규칙 우선순위 이해하기

규칙은 다음 순서로 선택됩니다.

1. 비활성화된 유형과 확장자가 다른 유형은 제외합니다.
2. 폴더 조건까지 일치하는 유형을 먼저 확인합니다. 지정한 폴더에는 그 하위 폴더도 포함됩니다.
3. 일치하는 폴더 지정 유형이 여러 개면 설정 목록에서 가장 위에 있는 유형을 적용합니다.
4. 일치하는 폴더 지정 유형이 없으면 폴더 조건이 없는 일반 유형 중 가장 위에 있는 유형을 적용합니다.

간단히 말하면 **일치하는 폴더 지정 유형 → 일치하는 일반 유형 → 아이콘 없음** 순서입니다. 같은 그룹의 규칙이 겹칠 때는 위·아래 버튼으로 어떤 유형을 우선할지 결정할 수 있습니다. 일반 유형을 폴더 지정 유형보다 위로 이동하더라도 일치하는 폴더 지정 유형이 우선합니다.

예를 들어 목록이 다음 순서라고 가정합니다.

```text
1. PDF 문서  | pdf       | 모든 폴더       | 📄
2. 연구 자료 | pdf       | Books/Research  | 🔬
3. 전자책    | pdf, epub | Books           | 📚
```

- `Books/Novel.pdf`에는 `전자책` 폴더 규칙만 일치하므로 `📚`가 표시됩니다.
- `Books/Research/Paper.pdf`에는 두 폴더 규칙이 모두 일치하지만 `연구 자료`가 목록에서 더 위에 있으므로 `🔬`가 표시됩니다.
- `Work/Report.pdf`에는 일치하는 폴더 규칙이 없으므로 일반 `PDF 문서`의 `📄`가 표시됩니다.

연구 PDF에도 `📚`를 표시하려면 `전자책`을 `연구 자료`보다 위로 이동하면 됩니다.
...
예를 들어 `Ebooks` 유형을 `Books` 폴더로 제한하면 다른 폴더의 PDF 아이콘은 그대로 유지할 수 있습니다.
